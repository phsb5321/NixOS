...
- `./user-scripts/nix-shell-selector.sh` - Interactive shell selector with
...
This is a modular NixOS flake configuration supporting multiple hosts with
...
- **media**: VLC, OBS, GIMP, Discord (spotifyd for Spotify via services module)
...
**IMPORTANT: This system is running as the SERVER host configuration. All
rebuilds should target the server configuration.**
...
- **Centralized ZSH Management**: All shell configuration is handled by the
  dedicated shell module
- **PowerLevel10k Theme**: Properly configured with instant prompt support and
  Nix store paths
- **Plugin System**: Modular plugin configuration (autosuggestions, syntax
  highlighting, you-should-use)
- **Modern Tools**: Integrated modern CLI replacements (eza, bat, fd, ripgrep,
  zoxide, etc.)
- **Per-Host Customization**: Shell features can be enabled/disabled per host
- **Clean Dotfiles**: Separated `.zshenv` (environment) from `.zshrc`
...
- Add new categories in `modules/packages/default.nix` following existing
  patterns
...
The GNOME configuration follows a **modular host-specific architecture** to
eliminate duplication while allowing complete customization per host.
...
Each host defines its **complete GNOME configuration** in
`hosts/<hostname>/gnome.nix`:
...
1. **No dconf Merging**: Each host defines one complete
...
2. **Shared Infrastructure**: Base services and packages defined once in
   `base.nix`
...
4. **Environment Isolation**: Host-specific environment variables override
   shared ones
...
- **Portal Integration**: Comprehensive XDG desktop portal configuration for
  file dialogs
...
- Use `nixswitch` for system rebuilds (handles validation, cleanup, error
  recovery)
